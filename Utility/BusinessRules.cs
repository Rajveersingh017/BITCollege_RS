...
namespace Utility   
{
    ...
    public static class BusinessRules
    {
        ...
        /// <returns>Error message that corresponds to error code.</returns>
        public static string RegisterError(int errorCode)
        {
            string returnMessage = "";
            if (errorCode == -100)
            {
                returnMessage = "Student cannot register for a course in which there is already an ungraded registration.";
            }
            else if(errorCode == -200)
            {
                returnMessage = "Student has exceeded maximum attempts on mastery course.";
            } 
            else if(errorCode == -300)
            {
                returnMessage = "An error has occurred while updating the registration.";
            }
            else
            {
        ...
        /// <summary>
        /// chops the string from the sentences to the point where word starts.
        /// </summary>
        /// <param name="sentence">string sentence</param>
        /// <param name="word">string word</param>
        /// <returns></returns>
        public static string chop (string sentence, string word)
        {
            return sentence.Substring(0, sentence.IndexOf(word));
        }
    }
}
